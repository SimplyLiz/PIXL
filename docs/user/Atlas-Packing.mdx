...
<Callout type="info">Composite sprites are packed into a separate atlas automatically — every variant and animation frame gets its own entry. Your sprite system can look them up by name (e.g., `knight:shield`, `knight:walk:2`).</Callout>
