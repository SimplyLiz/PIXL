...
<FeatureGrid>
  <Feature title="Tiled">`.tsj` tileset + `.tmj` tilemaps + PNG atlas — the most common level editor format</Feature>
  <Feature title="Godot">TileMap resources + PNG, ready to drop into Godot Engine</Feature>
  <Feature title="Unity">Tile palette data + PNG atlas for Unity's tilemap system</Feature>
  <Feature title="TexturePacker">JSON Hash metadata + PNG atlas — works with 48+ engines including Phaser, Pixi.js, libGDX, Bevy, and more</Feature>
  <Feature title="GB Studio">Game Boy compatible export for retro projects</Feature>
  <Feature title="PNG">Individual tile PNGs at any scale — works everywhere</Feature>
  <Feature title="GIF">Animated sprites with frame timing baked in</Feature>
</FeatureGrid>
...
Phaser, Pixi.js, libGDX, Cocos2d-x, HaxeFlixel, MonoGame, Bevy, Macroquad, LOVE, Defold, and dozens more.
